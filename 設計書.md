...
- **描画方式**: 状態変更→`renderEvents()`（リスト）／`renderCompare()`（比較）で `#timelineContainer` を innerHTML で全再構築。フラグ等の軽微な変更のみ `refreshCard()` で部分更新。
...
| 分類 | 関数 |
|---|---|
| 保存 | `loadJSON` / `safeSet` / `saveEvents` / `persistEvents` |
...
| 時間 | `normalizeTime` / `liveFormatTime`（入力中の自動`:`挿入） / `timeToMinutes` / `effectiveSortMinutes` |
...
| カード操作 | `startEditEvent`・`saveEditEvent`（フル編集）/ `startEditTime`・`saveEditTime`（時間だけ）/ `toggleEventFlag`・`toggleEventConceal`・`toggleEventColor` / `deleteEvent` / `moveUnknownEvent` / `toggleCardChar` |
...
| その他 | `applyView` / `setView` / `exportData` / `importData` / `clearAll*` / `toggleHelp` |
